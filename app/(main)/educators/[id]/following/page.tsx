...
export default function FollowingPage({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const [owner, following] = await Promise.all([
          getUser(id),
          getFollowing(id),
        ]);
        setPageOwner(owner);

        // Check which of these users the viewer already follows (skip self)
        ...
        setError("Unable to load following list. Please try again.");
      } finally {
        setLoading(false);
      }
    }
