...
export default function CommunityFeedPage() {
  return (
    <Suspense>
      <CommunityFeedPageInner />
    </Suspense>
  );
}

function CommunityFeedPageInner() {
  ...
  useEffect(() => {
    if (!authLoading) {
      void loadPosts(true, typeFilter);
    }
  ...
  async function loadMore() {
    ...
    void loadPosts(false, typeFilter);
  }
      ...
      <DiscoveryShell
        ...
        controls={
          <div className="space-y-4">
            {user && <CreatePost embedded onPostCreated={() => void loadPosts(true, typeFilter)} />}
        ...
        <div ref={sharedPostRef} className="scroll-mt-28">
          {sharedPostLoading ? (
            ...
            <div className="rounded-xl ring-1 ring-info-200">
              ...
              <PostCard post={sharedPost} textOnlyAvatars isAdmin={isAdmin} />
            </div>
          ) : (
          ...
          {showGuestWall && (
            <div className="rounded-xl border border-border bg-surface shadow-card p-8 text-center">
              ...
              <div className="mt-4 flex justify-center gap-3">
                <Button variant="primary" onClick={() => router.push("/auth/signup?redirect=/feed")}>
                  Create Account
                </Button>
                <Button variant="outline" onClick={() => router.push("/auth/login?redirect=/feed")}>
                  Sign In
                </Button>
              </div>
            </div>
