...
export default function ObjectivesStep({ state, onChange, errors = [] }: StepProps) {
  const objectivesError = errors.find((e) => e.field === "objectives")?.message;
  const { objectives } = state;
  const refs = useRef<Array<HTMLTextAreaElement | null>>([]);

  useEffect(() => {
    refs.current = refs.current.slice(0, objectives.length);
  }, [objectives.length]);

  function updateItem(index: number, value: string) {
    const next = objectives.map((item, i) => (i === index ? value : item));
  ...
  function handleKeyDown(e: KeyboardEvent<HTMLTextAreaElement>, index: number) {
    ...
    const hasValue = objectives[index].trim().length > 0;
  ...
  return (
    <div className="space-y-4">
      ...
      {objectivesError && (
        <p role="alert" className="text-xs text-error-600">{objectivesError}</p>
      )}
      ...
      <Button
        ...
        variant="ghost"
        size="sm"
        onClick={() => onChange({ objectives: [...objectives, ""] })}
      >
        + Add Objective
      </Button>
    </div>
